...
//   camera[9]     cy
//
// NOTE: custom_ba_core is intentionally identical to baseline_ba_core for now.
// Diverge the optimisation logic here as research progresses.
        ...
        bool   fix_points       = false)
{
    ...
    if (fix_points) {
        for (int i = 0; i < static_cast<int>(pt_buf.shape[0]); ++i) {
    ...
    // Use Schur-complement ordering: group 0 = 3-D points, group 1 = cameras.
    // This is the standard BA ordering that enables efficient elimination.
    auto* ordering = new ceres::ParameterBlockOrdering;
    for (int i = 0; i < static_cast<int>(pt_buf.shape[0]); ++i)
        ordering->AddElementToGroup(pt_data + 3 * i, 0);
    for (int i = 0; i < num_cams; ++i)
        ordering->AddElementToGroup(cam_data + 10 * i, 1);
